...
query_modifier_prompt = ChatPromptTemplate.from_messages([
    ("system", """
너는 RAG 시스템의 쿼리 교정 노드다.

역할:
사용자의 현재 질문(query)이 이전 대화 로그(memory)를 참조하는 불완전한 질문인지 판단하라.
불완전하다면, memory를 참고하여 독립적으로 이해할 수 있는 완전한 질문으로 재작성하라.

판단 기준:
- 대명사나 지시어("거기", "그것", "해당 사업", "위에서 말한" 등)가 이전 대화 없이는 해석 불가능한 경우 → 재작성
- 주어가 없는 경우 → 재작성
- query만으로 의미가 완전한 경우 → 그대로 반환

주의:
- 의미를 변경하거나 추가하지 마라. 빠진 맥락만 보충하라.
- 재작성이 필요하지만 memory가 없다면 그대로 반환하라.
- reasoning을 출력하지 마라.
...
query_modifier_chain = query_modifier_prompt | llm | JsonOutputParser()


def query_modifier_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    memory_for_llm = [
        {"log_id": log["log_id"], "query": log["query"], "final_answer": log["final_answer"]}
        for log in memory
    ]

    result = query_modifier_chain.invoke({
        "query": state["query"],
...
def build_rag_graph():
    ...
    builder.add_node("query_modifier", query_modifier_node)
    ...
    builder.set_entry_point('query_modifier')
    builder.add_edge('query_modifier', 'judge')
