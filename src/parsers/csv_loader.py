...
class CSVMarkdownConverter:
    ...
    def convert_row(self, row: dict[str, str]) -> MarkdownData:
        ...
        notice_num = row.get('공고 번호', '').strip()
        notice_round = row.get('공고 차수', '').strip()
        project_name = row.get('사업명', '').strip()
        amount = row.get('사업 금액', '').strip()
        org_name = remove_josa(row.get('발주 기관', '').strip())
        open_date = self._format_date(row.get('공개 일자', '').strip())
        start_date = self._format_date(row.get('입찰 참여 시작일', '').strip())
        end_date = self._format_date(row.get('입찰 참여 마감일', '').strip())
        summary = self._format_summary(row.get('사업 요약', '').strip())
        file_format = row.get('파일형식', '').strip()
        filename = row.get('파일명', '').strip()
        original_text = row.get('텍스트', '').strip()
