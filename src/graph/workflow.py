...
class RAGChatbotV17:
    ...
    def _is_csv_shortcircuit_eligible(self, query: str, intent: QueryIntent) -> bool:
        ...
        if field in {"open_date", "start_date", "end_date"}:
            if not any(token in normalized for token in ["입찰", "공고", "참여"]):
                return False
    ...
    def _select_csv_row_for_shortcircuit(
        ...
        for cand in [org_name, intent.org_name]:
            ...
            if name and name not in org_candidates:
        ...
        for cand in self._extract_org_names_from_query(query, limit=3, allow_project_fallback=False):
            ...
            if resolved and resolved not in org_candidates:
                org_candidates.append(resolved)
    ...
    def _try_csv_short_circuit(
        ...
        if not self._is_csv_shortcircuit_eligible(query, intent):
            return None
    ...
    def _hydrate_org_registry_from_existing_chunks(self) -> None:
        ...
        try:
            org_stats = self.vector_store.collect_org_stats()
        except Exception:
            return
        if not org_stats:
            return
    ...
    def _has_unindexed_document_files(self) -> bool:
        ...
        if not all_files:
            return False
        indexed_sources = self.vector_store.get_indexed_sources(doc_types=["pdf", "hwp"])
    ...
    def answer(self, query: str, top_k: int = 24) -> dict[str, Any]:
        ...
        for cand in explicit_orgs_raw:
            resolved = self._resolve_known_org_name(cand) or cand
            if resolved not in explicit_orgs:
                explicit_orgs.append(resolved)
        direct_explicit_orgs: list[str] = []
        for cand in direct_orgs_raw:
            resolved = self._resolve_known_org_name(cand) or cand
            if resolved not in direct_explicit_orgs:
                direct_explicit_orgs.append(resolved)
        explicit_org = explicit_orgs[0] if explicit_orgs else None
        if follow_up_ctx["is_follow_up"] and follow_up_ctx["last_org"] and not explicit_orgs:
    ...
    def _run_retrieval_call(
        ...
        if hasattr(self.vector_store, "search_hybrid"):
    ...
    def _org_names_loosely_match(left: str, right: str) -> bool:
        if not left or not right:
            return False
        left_key = re.sub(r"[^0-9a-zA-Z가-힣]+", "", unicodedata.normalize("NFKC", left.lower()))
        right_key = re.sub(r"[^0-9a-zA-Z가-힣]+", "", unicodedata.normalize("NFKC", right.lower()))
    ...
    def _extract_org_names_from_query(
        ...
        # 별칭 정규화 후 매칭
        normalized_query = self._normalize_legal_name_tokens(self.vector_store.normalize_org_name(query))
