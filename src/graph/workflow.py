...
class RAGChatbotV17:
    ...
    def answer(self, query: str, top_k: int = 24) -> dict[str, Any]:
        ...
        def _finalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
            ...
            if answer_text:
                payload["raw_answer"] = str(payload.get("raw_answer", "") or answer_text).strip()
                ...
                if polished_answer:
                    original_norm = re.sub(r"\s+", " ", answer_text).strip()
                    polished_norm = re.sub(r"\s+", " ", polished_answer).strip()
                    min_allowed = max(80, int(len(original_norm) * 0.85))
                    if len(polished_norm) >= min_allowed:
    ...
    def _build_answer_payload(
        ...
        raw_answer = str(draft.final_answer or "").strip()
        formatted_answer = RAGChatbotV17._format_answer_for_readability(raw_answer)
        return {
            "answer": raw_answer,
    ...
    def _format_answer_for_readability(answer: str, style: str = "concise") -> str:
        ...
        if not text:
            return ""
        readability_mode = unicodedata.normalize(
            "NFKC",
            str(os.environ.get("ANSWER_READABILITY_MODE", "lossless") or "lossless").strip().lower(),
        )
        if readability_mode in {"lossless", "full"}:
