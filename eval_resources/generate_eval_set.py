...
from src.utils.config import DEFAULT_MODEL, OPENAI_API_KEY
...
def _sample_chunks(collection, num_samples: int, min_length: int = 200) -> list[dict]:
    ...
    for i in range(len(ids)):
        ...
        candidates.append({
            ...
            "page": meta.get("page"),
            "institution": meta.get("institution") or meta.get("org", ""),
            "metadata": meta,
        })
...
def generate_eval_set(num_pairs: int = 20, model: str | None = None) -> list[dict]:
    """LLM 기반으로 평가셋을 생성한다."""
    load_dotenv()
    llm_model = model or os.getenv("DEFAULT_MODEL") or DEFAULT_MODEL
