...
- Plan 제공 여부와 관계없이 Work Done은 실제 수행한 항목을 전체 작성한다.
- Plan에 대응되는 Work Done 항목에만 `근거: ...`를 짧게 덧붙인다.
...
2. Plan 제공 시: `2. Work Done`부터 `5. Next Steps`까지만 작성한다.
3. Work Done은 Plan 대응 여부와 무관하게 실제 수행 항목을 전체 작성한다.
4. Plan에 대응되는 Work Done 항목에만 각 항목 끝에 `근거: ...`를 한 줄로 짧게 덧붙인다.
5. Plan 미제공 시: 오늘 대화에서 목표/실행/결과를 추출해 `1~5`를 모두 작성한다.
6. Insights는 재사용 가능한 교훈만 1~2개로 요약한다.
7. Issues & Fixes는 문제-원인-해결-결과를 한 세트로 쓰며, 해결한 문제 수에 따라 양식에 맞춰 늘려서 작성한다.

## 품질 체크
- 템플릿 섹션 누락 없음
- 입력 모드(Plan 제공/미제공)와 출력 범위가 일치함
- Work Done이 실제 수행 기준으로 누락 없이 작성됨
- Plan 대응 항목에만 `근거`가 포함됨
- 중복 문장 없음
- 추측성 문장 최소화
- 바로 복붙 가능한 상태
