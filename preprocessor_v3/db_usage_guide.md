...
**파이프라인 버전**: v3 (Regex Hierarchy + LangChain Splitters)  
...
| 단계 | 파일 | 설명 |
|------|------|------|
| Step 1 | `parser_step1.py` | PDF → Markdown (헤더 삽입 없음, `<<PAGE: N>>` 마커, 테이블 보존) |
| — | `auditor_step2.py` | 한국어 교정, 불릿 표준화, 공백 정리 |
| Step 2 | `chunker_step4.py` | 마크다운 테이블 → 자연어 평탄화 (`table_flattener`) |
| Step 3 | `chunker_step4.py` | 정규식 기반 섹션 계층 추출 (제N편/장/절/조/항, 숫자, 로마, 가-하, 괄호) |
| Step 4 | `chunker_step4.py` | `#` (level 1) / `##` (level 2) 헤더 삽입 (level 3+ 생략) |
| Step 5 | `chunker_step4.py` | 페이지 마커 변환 `<<PAGE: N>>` → `[[[Page: N]]]` |
| Step 6 | `chunker_step4.py` | `MarkdownHeaderTextSplitter` (헤더 기준 분할, 페이지 메타 추출) |
| Step 7 | `chunker_step4.py` | `RecursiveCharacterTextSplitter` (chunk_size=500, overlap=100) |
| — | `storage_step5.py` | DB 인덱싱 (Dense + Sparse + Hierarchy) |

**v2.2 대비 주요 변경**:
- 폰트 기반 헤더 탐지 → **정규식 기반 섹션 계층 추출**
- kiwipiepy 문장 분할 청킹 → **LangChain 텍스트 스플리터**
- 페이지 마커: `[[PAGE:N]]` → `<<PAGE: N>>`
- bracket 패턴 (`[별표1]` 등)은 항상 level 1 (최상위)
...
| `section_level1` | string | H1 섹션명 — 정규식 계층 level 1 (`"N/A"` = 해당 없음) |
| `section_level2` | string | H2 섹션명 — 정규식 계층 level 2 (`"N/A"` = 해당 없음) |
