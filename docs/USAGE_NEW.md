...
```bash
python -m src.rag_answer --query "한국농어촌공사 입찰 보증금 비율은 얼마인가?" --retriever chroma --chroma-persist-dir data_index/chroma_B --chroma-collection rfp_b_auto --chroma-model auto --rerank none --topk 50 --context-k 20 --generate --output-csv results/node_report_chroma_auto.csv
```
